...
module.exports.pancake = (version, modules, settings, GlobalSettings, cwd) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (typeof version !== 'string') {
      reject(`Plugin pancake-react got a mismatch for the data that was passed to it! ${Style.yellow(`version`)} was ${Style.yellow(typeof version)} ` + `but should have been ${Style.yellow(`string`)}`);
    }

    if (typeof modules !== 'object') {
      reject(`Plugin pancake-react got a mismatch for the data that was passed to it! ${Style.yellow(`modules`)} was ${Style.yellow(typeof modules)} ` + `but should have been ${Style.yellow(`object`)}`);
    }

    if (typeof settings !== 'object') {
      reject(`Plugin pancake-react got a mismatch for the data that was passed to it! ${Style.yellow(`settings`)} was ${Style.yellow(typeof settings)} ` + `but should have been ${Style.yellow(`object`)}`);
    }

    if (typeof cwd !== 'string') {
      reject(`Plugin pancake-react got a mismatch for the data that was passed to it! ${Style.yellow(`cwd`)} was ${Style.yellow(typeof cwd)} ` + `but should have been ${Style.yellow(`string`)}`);
    } //--------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    if (SETTINGS.react.location !== false) {
      for (const modulePackage of modules) {
        Log.verbose(`React: Building ${Style.yellow(modulePackage.name)}`); //check if there are react files
        ...
        if (!Fs.existsSync(reactModulePath)) {
          ...
          const reactModuleToPath = Path.normalize(`${cwd}/${SETTINGS.react.location}/${modulePackage.name.split('/')[1]}.js`); //move react file depending on settings

          const reactPromise = HandleReact(reactModulePath, reactModuleToPath, `${modulePackage.name} v${modulePackage.version}`).catch(error => {
            Log.error(error);
          });
      ...
      if (modules.length < 1) {
        ...
        Promise.all(reactModules).catch(error => {
          Loading.stop('pancake-react', Log.verboseMode); //stop loading animation
